...
const parseLayout = (layout) => {
  ...
  if (numCols % valueSum !== 0) {
    ...
    console.error(
      `Layout of ${layout} can't be expanded to fit a 12 column grid (sum is not divisor of 12).`
    );
    return undefined;
  }
...
const MdxLayoutAH = ({
  ...
  if (!layout) {
    ...
    defaultLayout = new Array(React.Children.count(children)).fill(
      Math.floor(numCols / React.Children.count(children))
    );
  }
  ...
  if (parsedLayout.length < React.Children.count(children)) {
    console.error(
      `Layout of ${layout} defines fewer columns than the number of provided children.`
    );
    return children;
  }
  ...
  const renderChildren = () => {
    return React.Children.map(children, (child, index) => {
      ...
      if (parsedTabletLayout) {
        classNameObject[`tablet-col-${parsedTabletLayout[index]}`] =
          !!parsedTabletLayout;
      }
      return (
        <div
  ...
  return (
    <div className={classnames("mdx-layout", classes)} {...props}>
      {renderChildren()}
    </div>
  );
};

MdxLayoutAH.propTypes = {
...
const MdxMedia = (props) => {
  return <MdxLayoutAH isMedia {...props} />;
};

export { MdxMedia, MdxLayoutAH };
